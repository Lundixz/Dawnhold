...
*   **Seam-Free Tiling (Grid Seams Fix)**: Fully resolved WebGL/WebGPU grid seams under camera zoom and panning. Symmetrical coordinates (`127.5, 63.5`) and a 1.5% asset extrusion (diamond threshold set to `1.015`) provide solid tile colors in a 2px horizontal / 1px vertical overlap region. This physical overlap of the visible diamonds completely hides any sub-pixel rendering gaps.
...
*   **4-Bit Isometric Shoreline Autotiling**: Integrated an advanced 4-bit neighbor lookup algorithm mapping North, South, East, and West connections. The terrain generator renders 14 custom, procedurally blended, rounded beach transition tiles (`beach_0001.png` to `beach_1110.png`) to eliminate checkered diamond grid patterns (waffle/cushion effects) and produce perfectly smooth, organic, curved shorelines where the sand gracefully meets the shallow and deep waters.
